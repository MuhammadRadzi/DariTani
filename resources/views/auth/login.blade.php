...
@section('content')
<div class="min-h-screen bg-brand-200 flex flex-col items-center justify-center px-4 py-10">
    ...
    <div class="bg-white w-full max-w-sm rounded-2xl shadow-lg px-4 py-10">
        ...
        <form method="POST" action="{{ route('login.submit') }}" class="flex flex-col gap-6 items-center">
            ...
            <button
                type="submit"
                class="bg-brand-300 hover:bg-brand-400 transition-colors w-full max-w-[302px] h-[47px] rounded-lg text-white text-lg font-medium"
            >
                Submit
            </button>
            ...
            <p class="text-sm text-[#5d5d5d]">
                Belum punya akun?
                <a href="{{ route('register') }}" class="text-brand-300 font-medium underline">Daftar</a>
            </p>
        </form>
    </div>
</div>
